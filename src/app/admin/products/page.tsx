...
function hrefFor(
  input: { q: string; status: string; featured: string; category: string },
  page: number,
) {
...
export default async function ProductsAdminPage({
  ...
  const [data, categories] = await Promise.all([
    listAdminProducts(await searchParams),
    listAdminCategories(),
  ]);
    ...
    <main className="mx-auto max-w-6xl p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage the Woodbay catalogue and public visibility.
          </p>
        </div>
        <Link
          href="/admin/products/new"
          className="woodbay-button inline-flex min-h-12 items-center justify-center rounded-[3px] px-6 py-3 text-[11px] font-medium tracking-[.14em] uppercase"
        >
          Add product
        </Link>
      </div>
      ...
      {data.rows.length === 0 ? (
        <p className="admin-empty mt-8 rounded-lg border bg-white">
          No products found. Try changing your filters or add a product.
        </p>
      ) : (
        <>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <AdminStatus
                    value={row.status}
                    label={
                      row.status === "draft" ? "Hidden / Draft" : undefined
                    }
                  />
                </div>
                <p className="text-muted-foreground mt-2 text-sm">
                  {categoryName(row.product_categories)} ·{" "}
                  {row.is_featured ? "Featured" : "Standard"}
                </p>
                ...
                {data.rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b last:border-0">
                      ...
                      <td className="p-3">{row.is_featured ? "Yes" : "—"}</td>
                      <td className="p-3">{adminDate(row.updated_at)}</td>
      ...
      {pageCount > 1 && (
        ...
        >
          <>
            {data.page > 1 ? (
              <Link href={hrefFor(query, data.page - 1)}>Previous</Link>
            ) : (
              <span />
            )}
          </>
          ...
          {data.page < pageCount ? (
            <Link href={hrefFor(query, data.page + 1)}>Next</Link>
          ) : (
            <span />
          )}
        </nav>
