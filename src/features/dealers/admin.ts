...
export async function getDealerApplication(id: string) {
  ...
  let dealer = null;
  if (data.dealer_id) {
    const result = await createAdminClient().from("dealers").select("id,business_name,contact_person,phone,email,state,district,area,address,status,is_visible,slug").eq("id", data.dealer_id).maybeSingle();
    dealer = result.data;
  }
...
export async function updateDealer(formData: FormData) {
  ...
  const values = z.object({
    ...
    district: z.string().trim().min(2).max(120), area: z.string().trim().max(160).optional(), address: z.string().trim().min(2).max(500),
  ...
  const { error } = await createAdminClient().from("dealers").update({ ...dealer, email: dealer.email || null, contact_person: dealer.contact_person || null, area: dealer.area || null }).eq("id", id);
