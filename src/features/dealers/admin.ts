...
function mapApplication(row: Record<string, unknown>): DealerApplicationRow {
  return {
    ...
    productInterests: Array.isArray(row.product_interests)
      ? row.product_interests.map(String)
      : [],
    status: row.status as DealerApplicationRow["status"],
...
export async function listDealerApplications(
  ...
  let query = db
    ...
    .select(
      "id,business_name,contact_person,phone,location,state,district,business_type,product_interests,status,created_at,dealer_id",
      { count: "exact" },
    )
  ...
  if (q)
    query = query.or(
      `business_name.ilike.%${q}%,contact_person.ilike.%${q}%,phone.ilike.%${q}%`,
    );
  if (location)
    query = query.or(
      `location.ilike.%${location}%,district.ilike.%${location}%`,
    );
...
export async function listDealers(input: Record<string, string | undefined>) {
  await requireAdmin();
  const q = (input.q ?? "").trim().slice(0, 120);
  const status = z
  ...
  let query = createAdminClient()
    .from("dealers")
    .select(
      "id,business_name,contact_person,phone,email,district,area,status,is_visible,payment_qr_image,dealer_applications(id)",
      { count: "exact" },
    )
    .order("updated_at", { ascending: false });
  if (q)
    query = query.or(
      `business_name.ilike.%${q}%,contact_person.ilike.%${q}%,phone.ilike.%${q}%,district.ilike.%${q}%`,
    );
  if (status !== "all") query = query.eq("status", status);
  const { data, error, count } = await query;
  if (error) throw new Error("Unable to load dealers.");
...
export async function getDealerApplication(id: string) {
  ...
  const { data, error } = await client
    .from("dealer_applications")
    .select(
      "id,business_name,contact_person,phone,whatsapp,email,state,district,location,address,business_type,years_in_business,has_showroom,areas_served,product_interests,message,status,admin_notes,created_at,updated_at,dealer_id",
    )
  ...
  if (data.dealer_id) {
    const result = await client
      .from("dealers")
      .select(
        "id,business_name,contact_person,phone,email,state,district,area,address,google_maps_url,latitude,longitude,payment_qr_image,shop_image,status,is_visible,slug",
      )
  ...
  const { data: audit } = await client
    .from("dealer_application_audit_events")
    .select("action,from_status,to_status,metadata,created_at")
...
export async function reviewDealerApplication(formData: FormData) {
  ...
  const client = createAdminClient();
  const { error } = await client.rpc("set_dealer_application_status", {
    p_application_id: id,
    p_status: status,
...
export async function createDealerFromApplication(formData: FormData) {
  ...
  const { error } = await createAdminClient().rpc(
    ...
    { p_application_id: id, p_actor_user_id: admin.userId },
  );
  if (error)
    throw new Error("Unable to create a dealer from this application.");
  revalidatePath("/admin/dealers");
  revalidatePath(`/admin/dealers/${id}`);
  revalidatePath("/dealers");
}
...
export async function updateDealer(formData: FormData) {
  ...
  const { error } = await createAdminClient()
    ...
    .update({
      ...
      contact_person: dealer.contact_person || null,
      area: dealer.area || null,
      google_maps_url: dealer.google_maps_url || null,
      latitude: Number.isFinite(dealer.latitude) ? dealer.latitude : null,
      longitude: Number.isFinite(dealer.longitude) ? dealer.longitude : null,
    })
    .eq("id", id);
...
export async function updateDealerAction(
  _previous: DealerActionState,
  formData: FormData,
): Promise<DealerActionState> {
  try {
    await updateDealer(formData);
    return { ok: true, message: "Dealer updated successfully." };
  } catch (error) {
    return {
      ok: false,
      message:
...
export async function uploadDealerImage(formData: FormData) {
  ...
  if (!(file instanceof File) || !(await isSafeImageUpload(file)))
    throw new Error("Use a valid JPG, PNG, WebP or AVIF image up to 10 MB.");
  const client = createAdminClient();
  const { data: current } = await client
    .from("dealers")
    .select("shop_image,payment_qr_image")
    .eq("id", dealerId)
    .maybeSingle();
  ...
  if (error) {
    ...
    throw new Error("Image uploaded but could not be attached to the dealer.");
  }
  const old = current[kind];
  const oldKey = old
    ? getExpectedMediaObjectKey(old, "dealers", dealerId)
    : null;
  if (oldKey) {
    try {
...
export async function removeDealerImage(formData: FormData) {
  ...
  const key = value
    ? getExpectedMediaObjectKey(value, "dealers", dealerId)
    : null;
  if (key) {
    try {
...
export async function deleteDealerApplication(formData: FormData) {
  ...
  if (application.dealer_id) {
    const { error } = await client
      .from("dealers")
      .update({ status: "inactive", is_visible: false })
      .eq("id", application.dealer_id);
    if (error)
      throw new Error(
        "This approved dealer was archived instead of permanently deleted.",
      );
  } else {
    const { error } = await client
      .from("dealer_applications")
...
export async function updateDealerApplicationNote(formData: FormData) {
  ...
  const notes = z
    ...
    .trim()
    .max(1000)
    .parse(formData.get("admin_notes") ?? "");
  const { error } = await createAdminClient()
    .from("dealer_applications")
...
export async function updateDealerApplication(formData: FormData) {
  ...
  const values = z
    .object({
      ...
      has_showroom: z.enum(["yes", "no"]),
      areas_served: z.string().trim().max(500),
      product_interests: z.array(z.string().trim().max(120)).max(4),
  ...
  const { error } = await createAdminClient()
    ...
    .update({
      ...
      has_showroom: values.has_showroom === "yes",
      areas_served: values.areas_served || null,
      product_interests: values.product_interests.length
