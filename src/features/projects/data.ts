...
export type ProjectPreview=Omit<Project,"project_images">;
const publicProjectListSelect="id,title,slug,category,location,description,featured_image";
const publicProjectSelect=`${publicProjectListSelect},project_images(id,storage_key,alt_text,sort_order)`;
...
export async function getPublishedProjectPreviews(){const{data,error}=await (await createClient()).from("projects").select(publicProjectListSelect).eq("status","published").order("sort_order").order("created_at",{ascending:false}).limit(5);if(error)throw error;return(data??[])as ProjectPreview[]}
